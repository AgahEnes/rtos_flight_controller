...
u8FrameCal = localBuildValidCalibrationFrame(c, uint8(11), uint32(1050), uint32(7));
u8FrameB = localBuildValidFrame(c, uint8(12), uint32(1100));
u8FrameCorrupt = u8FrameA;
u8FrameCorrupt(15) = bitxor(u8FrameCorrupt(15), uint8(1));

u8Noise = uint8([99 88 77 66 55 44 33]);
u8Stream = [u8Noise u8FrameCorrupt u8Noise u8FrameA u8Noise u8FrameCal u8Noise u8FrameB u8Noise];
...
assert(numel(frames) == 3, 'Expected 3 valid frames after resync.');
s1 = telem.decodeFrame(frames{1}, c);
sCal = telem.decodeCalibrationFrame(frames{2}, c);
s2 = telem.decodeFrame(frames{3}, c);
assert(s1.sequence == uint8(10), 'First sequence mismatch.');
assert(sCal.sequence == uint8(11), 'Calibration sequence mismatch.');
assert(sCal.updateCounter == uint32(7), 'Calibration update counter mismatch.');
assert(s2.sequence == uint8(12), 'Second sequence mismatch.');
...
function u8Frame = localBuildValidCalibrationFrame(c, seq, tsMs, updateCounter)
u8Frame = zeros(1, double(c.calibrationFrameLength), 'uint8');
u8Frame(1:4) = uint8([c.sync0 c.sync1 c.msgIdImuCalibration seq]);
u8Frame(5:8) = typecast(uint32(tsMs), 'uint8');
u8Frame(9:20) = typecast(single([0.01 -0.02 0.03]), 'uint8');
u8Frame(21:32) = typecast(single([0.11 -0.12 0.13]), 'uint8');
u8Frame(33:36) = typecast(uint32(2000), 'uint8');
u8Frame(37:40) = typecast(uint32(updateCounter), 'uint8');
u8Frame(41) = uint8(1);
