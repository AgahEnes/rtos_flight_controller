...
te_Bmp180_RetCode Bmp180_Stm32Hal_Read(uint8_t u8DeviceAddr,
                                        ...
                                        void *vpBusContext)
{
    ...
    {
        return BMP180_RET_INVALID_ARG;                       /* I2C handle, hedef buffer ve uzunluk olmadan HAL transferi güvenli değildir. */
    }
    ...
    if (eHalStatus != HAL_OK)
    {
        return ((eHalStatus == HAL_TIMEOUT) || (psContext->pxI2cHandle->ErrorCode == HAL_I2C_ERROR_TIMEOUT)) ?
                   BMP180_RET_TIMEOUT :
                   BMP180_RET_BUS_ERROR;                     /* HAL timeout status/error ayrı, diğer I2C/NACK/bus hataları genel bus error olarak döndürülür. */
    }

    return BMP180_RET_OK;                                    /* HAL okuma başarıyla tamamlanmıştır. */
}

te_Bmp180_RetCode Bmp180_Stm32Hal_Write(uint8_t u8DeviceAddr,
                                         uint8_t u8RegisterAddr,
                                         ...
                                         void *vpBusContext)
{
    ...
    {
        return BMP180_RET_INVALID_ARG;                       /* I2C handle, kaynak buffer ve uzunluk olmadan HAL yazması güvenli değildir. */
    }
    ...
    if (eHalStatus != HAL_OK)
    {
        return ((eHalStatus == HAL_TIMEOUT) || (psContext->pxI2cHandle->ErrorCode == HAL_I2C_ERROR_TIMEOUT)) ?
                   BMP180_RET_TIMEOUT :
                   BMP180_RET_BUS_ERROR;                     /* HAL timeout status/error ayrı raporlanır; diğer HAL hataları bus error olarak sadeleştirilir. */
    }

    return BMP180_RET_OK;                                    /* HAL yazma başarıyla tamamlanmıştır. */
}

te_Bmp180_RetCode Bmp180_Stm32Hal_DelayMs(uint32_t u32DelayMs, void *vpBusContext)
{
    (void)vpBusContext;                                      /* HAL_Delay ek bağlam istemez; callback imzası için parametre tutulur. */
    HAL_Delay(u32DelayMs);                                   /* BMP180 dönüşüm beklemeleri STM32 HAL zaman tabanı üzerinden yapılır. */
    return BMP180_RET_OK;                                    /* HAL_Delay dönüş değeri olmadığı için bekleme başarılı kabul edilir. */
}
...
te_Bmp180_RetCode Bmp180_Stm32Hal_Lock(uint32_t u32TimeoutMs, void *vpBusContext)
{
    ...
    if (psContext == NULL)
    {
        return BMP180_RET_INVALID_ARG;                       /* Mutex olmasa bile context pointer'ın geçerli olması port sözleşmesidir. */
    }

    if (psContext->xBusMutex == NULL)
    {
        return BMP180_RET_OK;                                /* Tek task veya harici korumalı kullanımda NULL mutex no-op kabul edilir. */
    }

    if (osMutexAcquire(psContext->xBusMutex, u32TimeoutMs) != osOK)
    {
        return BMP180_RET_TIMEOUT;                           /* Mutex alınamazsa paylaşılan I2C bus için bounded wait timeout raporlanır. */
    }

    return BMP180_RET_OK;                                    /* Mutex başarıyla alınmıştır; driver artık tek I2C transferini yapabilir. */
}

te_Bmp180_RetCode Bmp180_Stm32Hal_Unlock(void *vpBusContext)
{
    ...
    if (psContext == NULL)
    {
        return BMP180_RET_INVALID_ARG;                       /* Context yoksa hangi mutex'in bırakılacağı bilinemez. */
    }

    if (psContext->xBusMutex == NULL)
    {
        return BMP180_RET_OK;                                /* NULL mutex kullanılan sistemlerde unlock no-op kabul edilir. */
    }

    return (osMutexRelease(psContext->xBusMutex) == osOK) ?
               BMP180_RET_OK :
               BMP180_RET_INVALID_STATE;                     /* Release başarısızsa mutex sahipliği/durum problemi üst katmana taşınır. */
}

te_Bmp180_RetCode Bmp180_Stm32Hal_FillBusInterface(ts_Bmp180_BusInterface *psBusInterface,
                                                    ts_Bmp180_Stm32BusContext *psBusContext)
{
    if ((psBusInterface == NULL) || (psBusContext == NULL) || (psBusContext->pxI2cHandle == NULL))
    {
        return BMP180_RET_INVALID_ARG;                       /* Bus interface doldurmak için hedef struct ve geçerli I2C handle zorunludur. */
    }
    ...
    return BMP180_RET_OK;                                    /* Bus interface doldurma işlemi tamamlanmıştır. */
}

te_Bmp180_RetCode Bmp180_Stm32Hal_FillLockInterface(ts_Bmp180_LockInterface *psLockInterface,
                                                     ts_Bmp180_Stm32BusContext *psBusContext)
{
    if ((psLockInterface == NULL) || (psBusContext == NULL))
    {
        return BMP180_RET_INVALID_ARG;                       /* Lock interface doldurmak için hedef struct ve context zorunludur. */
    }
    ...
    return BMP180_RET_OK;                                    /* Lock interface doldurma işlemi tamamlanmıştır. */
}

te_Bmp180_RetCode Bmp180_Stm32Hal_FillTimingInterface(ts_Bmp180_TimingInterface *psTimingInterface)
{
    if (psTimingInterface == NULL)
    {
        return BMP180_RET_INVALID_ARG;                       /* Timing interface doldurmak için hedef struct zorunludur. */
    }

    psTimingInterface->pfnDelayMs = Bmp180_Stm32Hal_DelayMs; /* Conversion wait callback'i HAL_Delay adaptörüne bağlanır. */
    ...
    return BMP180_RET_OK;                                    /* Timing interface doldurma işlemi tamamlanmıştır. */
}
