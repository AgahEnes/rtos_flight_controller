...
static char *Bmp180_Test_prvAppendS32(char *pcDst, int32_t s32Value)
{
    uint32_t u32Abs;                                         /* İşaret ayrıldıktan sonra yazılacak mutlak değeri tutar. */

    if (s32Value < 0)
    {
        pcDst = Bmp180_Test_prvAppendChar(pcDst, '-');       /* Negatif değerler için önce eksi işareti eklenir. */
        u32Abs = (uint32_t)(-s32Value);                      /* Mutlak değer decimal yazım helper'ına verilir. */
    }
    else
    {
        u32Abs = (uint32_t)s32Value;                         /* Pozitif değerler doğrudan unsigned yazım helper'ına verilir. */
    }

    return Bmp180_Test_prvAppendU32(pcDst, u32Abs);          /* Decimal sayı metne eklenir. */
}
...
static void Bmp180_Test_prvSendSample(UART_HandleTypeDef *pxUart, const ts_Bmp180_Data *psData)
{
    ...
    pcWrite = Bmp180_Test_prvAppendText(pcWrite, " m RawUT="); /* Debug ham sıcaklık etiketi eklenir. */
    pcWrite = Bmp180_Test_prvAppendS32(pcWrite, psData->s32RawTemperature); /* Ham UT değeri yazılır. */
    pcWrite = Bmp180_Test_prvAppendText(pcWrite, " RawUP="); /* Debug ham basınç etiketi eklenir. */
    pcWrite = Bmp180_Test_prvAppendS32(pcWrite, psData->s32RawPressure); /* Ham UP değeri yazılır. */
...
int8_t BMP180_TEST(const ts_Bmp180_TestParams *psX)
{
    ...
    te_Bmp180_RetCode eRet;                                  /* Driver ve port çağrılarının dönüş kodunu taşır. */
    ...
    (void)Bmp180_Stm32Hal_FillTimingInterface(&sTimingIf);   /* HAL_Delay/HAL_GetTick callback'leri interface struct'ına doldurulur. */
    ...
    {
        eRet = Bmp180_Read(&sHandle, &sData);                /* Her döngüde sıcaklık, basınç ve altitude ölçümü alınır. */
        if (eRet != BMP180_RET_OK)
        {
            Bmp180_Test_prvSendText(psX->pxUart, "BMP180_TEST: Read failed\r\n"); /* Ölçüm hatası UART'a bildirilir. */
