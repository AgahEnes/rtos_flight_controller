...
{
    ts_TopicActuatorCmd sZeroTopic;

    (void)memset(&sZeroTopic, 0, sizeof(sZeroTopic));
    (void)memset(&gsActuatorCmdStorage, 0, sizeof(gsActuatorCmdStorage));
    gsActuatorCmdStorage.asActuatorCmdBuffers[0] = sZeroTopic;
...
te_GdsRetCode Gds_PublishActuatorCmd(const ts_TopicActuatorCmd *psCommand)
{
    ...
    (void)atomic_fetch_add_explicit(&gsActuatorCmdStorage.u32SeqLock, 1U, memory_order_acq_rel);

    u8ActiveIdx = (uint8_t)atomic_load_explicit(&gsActuatorCmdStorage.u8ActiveBufferIdx, memory_order_acquire);
    u8WriteIdx = (uint8_t)((uint8_t)1U - u8ActiveIdx);

    gsActuatorCmdStorage.asActuatorCmdBuffers[u8WriteIdx] = *psCommand;

    (void)atomic_store_explicit(&gsActuatorCmdStorage.u8ActiveBufferIdx, u8WriteIdx, memory_order_release);
    (void)atomic_fetch_add_explicit(&gsActuatorCmdStorage.u32SeqLock, 1U, memory_order_acq_rel);
...
te_GdsRetCode Gds_ReadActuatorCmd(ts_TopicActuatorCmd *psCommand)
{
    ...
    for (u32RetryCount = 0U; u32RetryCount < 3U; u32RetryCount++)
    {
        u32SeqStart = (uint32_t)atomic_load_explicit(&gsActuatorCmdStorage.u32SeqLock, memory_order_acquire);

        u8ActiveIdx = (uint8_t)atomic_load_explicit(&gsActuatorCmdStorage.u8ActiveBufferIdx, memory_order_acquire);
        *psCommand = gsActuatorCmdStorage.asActuatorCmdBuffers[u8ActiveIdx];

        u32SeqEnd = (uint32_t)atomic_load_explicit(&gsActuatorCmdStorage.u32SeqLock, memory_order_acquire);
        if ((u32SeqStart == u32SeqEnd) && ((u32SeqEnd & 1U) == 0U))
        {
            return GDS_OK;
        }
